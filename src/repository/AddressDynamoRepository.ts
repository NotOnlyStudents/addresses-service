...
class AddressDynamoRepository implements AddressRepository {
    ...
    async getAddress(userId: string, addrId: string): Promise<Address> {
        try {
            const address = await this.mapper.get(Object.assign(new AddressWithDynamoAnnotations, { id: addrId, username: userId }));
    ...
    async deleteAddress(userId: string, addrId: string): Promise<Address> {
        return this.mapper.delete(Object.assign(new AddressWithDynamoAnnotations, { id: addrId, username: userId }));
    }
